...
void append_padding_bits() {
    endd = static_cast<uint8_t*>(input) + saved_length;
    *endd = 0x80;
    ++endd;

    while ((endd - static_cast<uint8_t*>(input)) % 64 != 56)
        *endd = 0x00, ++endd;
}

void append_length() {
    uint64_t length = saved_length * 8;
    memcpy(static_cast<void*>(endd), &length, 8);

    if (endian() == BIG_ENDIAN) {
        std::swap(*endd, *(endd + 7));
        std::swap(*(endd + 1), *(endd + 6));
        std::swap(*(endd + 2), *(endd + 5));
        std::swap(*(endd + 3), *(endd + 4));
    }
